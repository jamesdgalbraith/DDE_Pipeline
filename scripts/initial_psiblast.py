...
def psiblast(x):
  ...
  if args.db_type == 'prot':
    os.system('psiblast -in_pssm '+out_path+'/pssms/'+z+'.pssm -db '+args.database+' -out '+out_path+'/xml/'+z+'.xml -num_threads '+str(args.blast_threads)+' -num_iterations '+str(args.iterations)+' -outfmt 5 -max_target_seqs 999999 -evalue 0.05 -out_pssm '+out_path+'/new_pssms/'+z+'.pssm')
  else:
